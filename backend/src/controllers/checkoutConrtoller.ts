...
import { checkoutSession, CheckoutSessionLine, products } from "../db/schema";
...
export async function createCheckout(
	...
	try {
		...
		if (totalCents < 10) {
			res.status(400).json({
				error: "Total below Polar minimum(e.g. USD requires at least 10 centa)",
			});
		}
